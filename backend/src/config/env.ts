...
const envSchema = z.object({
  ...
  FLIGHT_PROVIDER: z.enum(["mock", "serpapi"]).default("mock"),
  SCHEDULED_FLIGHT_PROVIDER: z.enum(["mock", "serpapi"]).default("mock"),
  ...
  RATE_LIMIT_WINDOW_MINUTES: z.coerce.number().int().positive().max(1440).default(15),
  FLIGHT_SEARCH_RATE_LIMIT: z.coerce.number().int().positive().max(1000).default(5),
  BOOKING_PRICE_RATE_LIMIT: z.coerce.number().int().positive().max(1000).default(20),
  TRIP_ASSISTANT_RATE_LIMIT: z.coerce.number().int().positive().max(1000).default(30),
