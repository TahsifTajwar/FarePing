...
export function runMockFlightSearch(search: FlightSearchInput) {
  ...
  return filterAndSortByBudget(results, search.maxPrice);
}
...
function buildRoundTripResults(
  ...
  const splitOneWays: MockItinerary = {
...
function filterAndSortByBudget(results: MockItinerary[], maxPrice: number) {
  return results
